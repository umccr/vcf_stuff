...
def compare_to_rm(method, vrn_file, rm_file, rm_interval_file):
    """Compare final variant calls against reference materials of known calls.
    """
    validate = None
    if not vcf_has_variants(vrn_file):
        # RTG can fail on totally empty files. Skip these since we have nothing.
        pass
    # empty validation file, every call is a false positive
    elif not vcf_has_variants(rm_file):
        eval_files = _setup_call_fps(vrn_file, rm_interval_file)
        validate = _rtg_add_summary_file(eval_files)
    elif method == "rtg":
        eval_files = _run_rtg_eval(vrn_file, rm_file, rm_interval_file)
        eval_files = _annotate_validations(eval_files)
        validate = _rtg_add_summary_file(eval_files)
    elif method == "hap.py":
        validate = _run_happy_eval(vrn_file, rm_file, rm_interval_file)
    return validate

...
library(modelr)
ml <- loess(af ~ hp, zh)
zh <- add_predictions(zh, ml, var = "pred_af")
ggplot(zh) + geom_point(aes(hp, af)) + geom_point(aes(hp, pred_af), color = "red")
zh %>% mutate(pred_af = )
...
load_gene_blacklists <- function() {
  folder <- "/Users/vsaveliev/git/NGS_Reporting/variant_filtering/reference_data/incidentalome"
  files <- list()
  files['Freq mut gene in HGMD'] = 'published/flags_in_hgmd.txt'
  files['Freq mut gene in OMIM'] = 'published/flags_in_omim.txt'
  files['Freq mut gene'] = 'published/flags.txt'
  files['Incidentalome gene'] = 'published/incidentalome.txt'
  files['MutSigCV gene'] = 'published/mutsigcv.txt'
  files['Low complexity gene'] = 'low_complexity/low_complexity_entire_gene.txt'
  files['Repetitive single exon gene'] = 'low_complexity/repetitive_single_exon_gene.txt'
  files['Low GC gene'] = 'low_complexity/low_gc.txt'
  files['High GC gene'] = 'low_complexity/high_gc.txt'
  files['Gene with too many COSMIC mutations'] = 'low_complexity/too_many_cosmic_mutations.txt'
  
  tbl = tibble(gene = "", blacklist = "")
  for (reason in names(files)) {
    fp <- file.path(folder, files[[reason]])
    gs <- read_tsv(fp, col_types = "c")
    for (g in gs) {
      tbl <- add_row(tbl, gene = g, blacklist = reason)
    }
  }
  tbl %>% 
    group_by(gene) %>% 
...
blacklists <- load_gene_blacklists()

with_bl <- data_p %>%
  left_join(blacklists, by = c("PCGR_SYMBOL" = "gene"))

names(with_bl)
with_bl %>% 
  filter(FIL.t != "PASS" | !is.na(blacklists)) %>% 
  select(C, POSITION, CH, FIL.t, HS, GIAB_CONF, PoN_CNT, PCGR_SYMBOL, TRICKY, ENCODE, HMF_MAPPABILITY, HP_ELEMENT, GC.t, blacklists) %>% 
  datatable()
